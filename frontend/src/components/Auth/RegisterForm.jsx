...
const RegisterForm = () => {
  ...
  const googleLoginTrigger = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      setFormError('');
      setSubmitting(true);
      const result = await loginWithGoogle(tokenResponse.access_token, selectedRole);
      setSubmitting(false);
    ...
    onError: () => {
      setFormError('El registro con Google falló.');
    }
  });
  ...
  const handleGoogleClick = () => {
    Swal.fire({
      ...
      text: '¿Con qué rol deseas registrarte en el Ecosistema?',
      input: 'select',
      inputOptions: {
        '2': '🚀 Emprendedor / Startup',
        '3': '⚡ Aceleradora de Startups',
        '4': '💼 Inversionista / Mentor'
      },
      ...
      inputValidator: (value) => {
        return new Promise((resolve) => {
          if (value) {
            ...
            resolve('Debes seleccionar un rol para continuar.');
          }
        });
      }
    }).then((result) => {
      ...
      Swal.fire({
        ...
        html: `
          <div style="text-align:left; line-height:1.6; font-size:14px;">
            ...
            <p>Para terminar el acceso, te enviamos un codigo temporal al correo:</p>
            <p style="font-weight:700; color:#b1f500;">${email}</p>
            <p>En la siguiente pantalla podras validarlo o reenviarlo.</p>
          </div>
        `,
        icon: 'success',
        confirmButtonText: 'Ir a verificacion',
        confirmButtonColor: '#7900c2',
        background: '#0b1324',
        color: '#fff',
        allowOutsideClick: false
      ...
      <div className="auth-form-panel">
        <div className="auth-content">
          ...
          <form className="auth-form" onSubmit={handleSubmit}>
            ...
            {/* Registro con Google */}
            <button type="button" className="social-btn" style={{ '--delay': '0.9s', marginTop: '15px' }} onClick={handleGoogleClick}>
