...
beforeAll(async () => {
    ...
    await sequelize.models.User.create({
        id: 2,
        cedula: 'eco5678',
        nombre_hacienda: 'Eco User 2',
        email: 'eco2@example.com',
        password_hash: 'secure123',
        role_id: 2
    });
    // Seed startup
    await sequelize.models.Startup.create({
        id: 1,
        user_id: 2,
        nombre_comercial: 'EcoStartup',
        descripcion: 'Startup for ecosystem test',
        fase: 'Idea',
        sector_id: 1
    });
});
...
describe('Solicitudes CRUD API', () => {
    ...
    it('Debería actualizar geolocalizacion', async () => {
        const res = await request(app).put(`/api/ecosistemas/editar-ecosystem/${createdGeoId}`).send({
